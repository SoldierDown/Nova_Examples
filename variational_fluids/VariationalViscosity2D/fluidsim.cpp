...
#else
	estimate_volume_fractions(double_size_grid, Vec3f(0.25f * dx, 0.25f * dx, 0.25f * dx), 0.5f * dx, liquid_phi_nodal, Vec3f(0, 0, 0), dx);
#endif
...
void FluidSim::compute_phi()
{
...
#ifdef UM
	// grab from particles
	liquid_phi_nodal.assign(3 * dx);
	Vec3f origin(-.5f * dx, -.5f * dx, -.5 * dx);
	for (unsigned int p = 0; p < particles.size(); ++p)
	{
		Vec3i node_ind((particles[p] - origin) / dx);
		for (int k = max(0, node_ind[2] - 1); k <= min(node_ind[2] + 1, nk); ++k)
		{
			for (int j = max(0, node_ind[1] - 1); j <= min(node_ind[1] + 1, nj); ++j)
			{
				for (int i = max(0, node_ind[0] - 1); i <= min(node_ind[0] + 1, ni); ++i)
				{
					Vec3f sample_pos(i * dx, j * dx, k * dx);
					float test_val = dist(sample_pos, particles[p]) - particle_radius;
					if (test_val < liquid_phi_nodal(i, j, k))
						liquid_phi_nodal(i, j, k) = test_val;
				}
			}
		}
	}
	...
	// extend phi slightly into solids (this is a simple, naive approach, but works reasonably well)
	Array3f phi_nodal_temp = liquid_phi_nodal;
	for (int k = 0; k < nk + 1; ++k)
	{
		for (int j = 0; j < nj + 1; ++j)
		{
			for (int i = 0; i < ni + 1; ++i)
			{
				if (liquid_phi_nodal(i, j, k) < 0.5 * dx)
				{
					float solid_phi_val = nodal_solid_phi(i, j, k);
					if (solid_phi_val < 0)
						phi_nodal_temp(i, j, k) = -0.5f * dx;
				}
			}
		}
	}
	liquid_phi_nodal = phi_nodal_temp;
#endif
}
