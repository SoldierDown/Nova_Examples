										 ...
										 const Array3f &viscosity, float dt, float dx)
{
	...
	*/
	printf("Set up identity rows before solving the system.\n");
	bool to_print = false;
	for (int row = 0; row < matrix.index.size(); ++row)
	{
		if (matrix.index[row].size() == 0)
		{
			matrix.set_element(row, row, 1.);
			if (row < (nx + 1) * ny * nz)
			{
				int crow = row;
				int kk = crow / ((nx + 1) * ny);
				int jj = (crow - kk * (nx + 1) * ny) / (nx + 1);
				int ii = crow - kk * (nx + 1) * ny - jj * (nx + 1);
				rhs[row] = u(ii, jj, kk);
			}
			else if (row < (nx + 1) * ny * nz + nx * (ny + 1) * nz)
			{
				int crow = row - (nx + 1) * ny * nz;
				int kk = crow / (nx * (ny + 1));
				int jj = (crow - kk * nx * (ny + 1)) / nx;
				int ii = crow - kk * nx * (ny + 1) - jj * nx;
				rhs[row] = v(ii, jj, kk);
			}
			else
			{
				int crow = row - (nx + 1) * ny * nz - nx * (ny + 1) * nz;
				int kk = crow / (nx * ny);
				int jj = (crow - kk * nx * ny) / nx;
				int ii = crow - kk * nx * ny - jj * nx;
				rhs[row] = w(ii, jj, kk);
			}
		}
		else
