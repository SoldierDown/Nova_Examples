...
void FluidSim::initialize(float width, int ni_, int nj_, int nk_)
{
	...
	cell_liquid_phi.resize(ni, nj, nk);
	nodal_liquid_phi.resize(ni + 1, nj + 1, nk + 1);

	// set viscosity
	viscosity.resize(ni, nj, nk, 1.f);
...
void FluidSim::compute_viscosity_weights()
{
	...
	estimate_volume_fractions(double_size_grid, Vec3f(0.25f * dx, 0.25f * dx, 0.25f * dx), 0.5f * dx, nodal_liquid_phi, Vec3f(0, 0, 0), dx);
...
void FluidSim::compute_phi()
{
		...
		for (int k = max(0, cell_ind[2] - 1); k <= min(cell_ind[2] + 1, nk - 1); ++k)
		{
			for (int j = max(0, cell_ind[1] - 1); j <= min(cell_ind[1] + 1, nj - 1); ++j)
			{
				for (int i = max(0, cell_ind[0] - 1); i <= min(cell_ind[0] + 1, ni - 1); ++i)
				{
					...
					if (test_val < cell_liquid_phi(i, j, k))
						cell_liquid_phi(i, j, k) = test_val;
				}
			}
		}
	}
	...
	Array3f phi_temp = cell_liquid_phi;
	for (int k = 0; k < nk; ++k)
	{
		for (int j = 0; j < nj; ++j)
		{
			for (int i = 0; i < ni; ++i)
			{
				if (cell_liquid_phi(i, j, k) < 0.5 * dx)
				{
					float solid_phi_val = 0.125f * (nodal_solid_phi(i, j, k) + nodal_solid_phi(i + 1, j, k) + nodal_solid_phi(i, j + 1, k) + nodal_solid_phi(i + 1, j + 1, k) + nodal_solid_phi(i, j, k + 1) + nodal_solid_phi(i + 1, j, k + 1) + nodal_solid_phi(i, j + 1, k + 1) + nodal_solid_phi(i + 1, j + 1, k + 1));
					if (solid_phi_val < 0)
	...
	cell_liquid_phi = phi_temp;

	// grab from particles
	nodal_liquid_phi.assign(3 * dx);
	for (unsigned int p = 0; p < particles.size(); ++p)
	{
		Vec3i cell_ind(particles[p] / dx);
		for (int k = max(0, cell_ind[2] - 1); k <= min(cell_ind[2] + 1, nk - 1); ++k)
		{
			for (int j = max(0, cell_ind[1] - 1); j <= min(cell_ind[1] + 1, nj - 1); ++j)
			{
				for (int i = max(0, cell_ind[0] - 1); i <= min(cell_ind[0] + 1, ni - 1); ++i)
				{
					Vec3f sample_pos(i * dx, j * dx, k * dx);
					float test_val = dist(sample_pos, particles[p]) - particle_radius;
					if (test_val < nodal_liquid_phi(i, j, k))
						nodal_liquid_phi(i, j, k) = test_val;
				}
			}
		}
	}
	...
	// extend phi slightly into solids (this is a simple, naive approach, but works reasonably well)
	Array3f nodal_phi_temp = nodal_liquid_phi;
	for (int k = 0; k < nk + 1; ++k)
	{
		for (int j = 0; j < nj + 1; ++j)
		{
			for (int i = 0; i < ni + 1; ++i)
			{
				if (nodal_liquid_phi(i, j, k) < 0.5 * dx)
				{
					float solid_phi_val = nodal_solid_phi(i, j, k);
					if (solid_phi_val < 0)
						nodal_phi_temp(i, j, k) = -0.5f * dx;
				}
			}
		}
	}
	nodal_liquid_phi = nodal_phi_temp;
}
...
void FluidSim::solve_pressure(float dt)
{
	...
	for (int k = 1; k < nk - 1; ++k)
	{
		for (int j = 1; j < nj - 1; ++j)
		{
			for (int i = 1; i < ni - 1; ++i)
			{
				...
				rhs[index] = 0;
				pressure[index] = 0;
				float centre_phi = cell_liquid_phi(i, j, k);
				if (centre_phi < 0)
				{

					// right neighbour
					float term = u_weights(i + 1, j, k) * dt / sqr(dx);
					float right_phi = cell_liquid_phi(i + 1, j, k);
					if (right_phi < 0)
					{
					...
					float left_phi = cell_liquid_phi(i - 1, j, k);
					if (left_phi < 0)
					{
						matrix.add_to_element(index, index, term);
						matrix.add_to_element(index, index - 1, -term);
					...
					float top_phi = cell_liquid_phi(i, j + 1, k);
					if (top_phi < 0)
					{
						matrix.add_to_element(index, index, term);
					...
					float bot_phi = cell_liquid_phi(i, j - 1, k);
					if (bot_phi < 0)
					{
						matrix.add_to_element(index, index, term);
						matrix.add_to_element(index, index - ni, -term);
					...
					float far_phi = cell_liquid_phi(i, j, k + 1);
					if (far_phi < 0)
					{
						matrix.add_to_element(index, index, term);
					...
					float near_phi = cell_liquid_phi(i, j, k - 1);
					if (near_phi < 0)
					{
						matrix.add_to_element(index, index, term);
						matrix.add_to_element(index, index - ni * nj, -term);
	...
	for (int k = 0; k < u.nk; ++k)
		for (int j = 0; j < u.nj; ++j)
			for (int i = 1; i < u.ni - 1; ++i)
			{
				...
				if (u_weights(i, j, k) > 0 && (cell_liquid_phi(i, j, k) < 0 || cell_liquid_phi(i - 1, j, k) < 0))
				{
					float theta = 1;
					if (cell_liquid_phi(i, j, k) >= 0 || cell_liquid_phi(i - 1, j, k) >= 0)
						theta = fraction_inside(cell_liquid_phi(i - 1, j, k), cell_liquid_phi(i, j, k));
					if (theta < 0.01f)
						theta = 0.01f;
	...
	for (int k = 0; k < v.nk; ++k)
		for (int j = 1; j < v.nj - 1; ++j)
			...
			{
				int index = i + j * ni + k * ni * nj;
				if (v_weights(i, j, k) > 0 && (cell_liquid_phi(i, j, k) < 0 || cell_liquid_phi(i, j - 1, k) < 0))
				{
					float theta = 1;
					if (cell_liquid_phi(i, j, k) >= 0 || cell_liquid_phi(i, j - 1, k) >= 0)
						theta = fraction_inside(cell_liquid_phi(i, j - 1, k), cell_liquid_phi(i, j, k));
					if (theta < 0.01f)
						theta = 0.01f;
	...
	for (int k = 1; k < w.nk - 1; ++k)
		for (int j = 0; j < w.nj; ++j)
			...
			{
				int index = i + j * ni + k * ni * nj;
				if (w_weights(i, j, k) > 0 && (cell_liquid_phi(i, j, k) < 0 || cell_liquid_phi(i, j, k - 1) < 0))
				{
					float theta = 1;
					if (cell_liquid_phi(i, j, k) >= 0 || cell_liquid_phi(i, j, k - 1) >= 0)
						theta = fraction_inside(cell_liquid_phi(i, j, k - 1), cell_liquid_phi(i, j, k));
					if (theta < 0.01f)
						theta = 0.01f;
